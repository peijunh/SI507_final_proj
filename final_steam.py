...
def make_move(player, board):
    ...
    player_move = input(player_name(player) + "'s move: ")
    while True:
        if len(player_move) != 1 or player_move <'1' or player_move > '9': #check if the input is legal
            print("Enter a number between 1 and 9.")
            player_move = input(player_name(player) + "'s move: ")
        elif board[int(player_move)-1] != 0: #check if the space is taken
            print("That space is occupied, try another.")
            player_move = input(player_name(player) + "'s move: ")
        else:
            break
    board[int(player_move)-1] = player

    pass


def check_win_horizontal(board):
    '''checks a board to see if there's a winner in a horizontal line

    scan the first row (index 0,1,2), second row (index 3,4,5), third row (index 6,7,8)
    if the three space of one of them is occupied purely by a player, then report a win

    Parameters
    ----------
    board: list
        the board where we check if there is a winner
...
def check_win_vertical(board):
    '''checks a board to see if there's a winner in a vertical line

    scan the first column (index 0,3,6), second column (index 1,4,7), third column (index 2,5,8)
    if the three space of one of them is occupied purely by a player, then report a win
...
def check_win_diagonal(board):
    '''checks a board to see if there's a winner in a diagonal line

    scan the first diagonal (index 0,4,8) and second diagonal (index 2,4,6)
    if the three space of one of them is occupied purely by a player, then report a win
